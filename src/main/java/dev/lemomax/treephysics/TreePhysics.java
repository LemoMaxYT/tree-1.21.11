...
public final class TreePhysics implements ModInitializer {
    ...
    private static void onLogBroken(World world, PlayerEntity player, BlockPos brokenPos,
                                     BlockState brokenState, net.minecraft.block.entity.BlockEntity blockEntity) {
        ...
        for (BlockPos pos : logs) {
            ...
            serverWorld.setBlockState(pos, net.minecraft.block.Blocks.AIR.getDefaultState(), 3);
            FallingBlockEntity falling = FallingBlockEntity.spawnFromBlock(serverWorld, pos, state);
            falling.setVelocity(push);
        }
    }

    private static Set<BlockPos> collectConnectedLogs(ServerWorld world, BlockPos start) {
        ...
        int minY = Math.max(world.getBottomY(), start.getY() - MAX_DISTANCE);
        int maxY = Math.min(world.getTopYInclusive(), start.getY() + MAX_DISTANCE);
        ...
        while (!queue.isEmpty() && result.size() < MAX_LOGS) {
            ...
            if (!world.getBlockState(pos).isIn(BlockTags.LOGS) && !pos.equals(start)) {
                continue;
            }
            ...
            for (Direction direction : Direction.values()) {
                ...
                if (!result.contains(next) && world.getBlockState(next).isIn(BlockTags.LOGS)) {
    ...
    private static Vec3d calculateFallDirection(PlayerEntity player, BlockPos brokenPos) {
        ...
        return new Vec3d(horizontal.x, -0.02, horizontal.z);
    }
}
